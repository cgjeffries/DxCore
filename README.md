...
### Low Power Modes
Functions are provided for causing the microcontroller to go to a low-power mode, disabling/enabling the input buffers, disabling/enabling the peripherals, disabling the floating pins, and making the microcontroller sleep for a set amount of time in order to reduce the microcontroller's power draw. These functions are documented in [megaavr/extras/LowPower.md](megaavr/extras/LowPower.md).
